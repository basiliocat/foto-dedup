"""Tests for fotodedup.crossdupes module."""
...
from fotodedup.crossdupes import (
...
# --- find_cross_dir_dupes with ext filter ---

def test_find_dupes_ext_filter_includes_matching(conn):
    """ext filter: only files matching extension are returned."""
    _insert(conn, "/a/photo.jpg", "/a", "photo.jpg", 1000, "abc")
    _insert(conn, "/b/photo.jpg", "/b", "photo.jpg", 1000, "abc")
    _insert(conn, "/a/doc.txt", "/a", "doc.txt", 1000, "abc")
    _insert(conn, "/b/doc.txt", "/b", "doc.txt", 1000, "abc")

    groups = find_cross_dir_dupes(conn, ext={".jpg"})
    assert len(groups) == 1
    key, files = groups[0]
    assert key["filename"] == "photo.jpg"


def test_find_dupes_ext_filter_excludes_nonmatching(conn):
    """ext filter: files not matching extension are excluded."""
    _insert(conn, "/a/doc.txt", "/a", "doc.txt", 1000, "abc")
    _insert(conn, "/b/doc.txt", "/b", "doc.txt", 1000, "abc")
...
def test_find_dupes_ext_none_includes_all(conn):
    """ext=None means no filtering — all extensions included."""
    _insert(conn, "/a/photo.jpg", "/a", "photo.jpg", 1000, "abc")
    _insert(conn, "/b/photo.jpg", "/b", "photo.jpg", 1000, "abc")
    _insert(conn, "/a/doc.txt", "/a", "doc.txt", 2000, "xyz")
    _insert(conn, "/b/doc.txt", "/b", "doc.txt", 2000, "xyz")

    groups = find_cross_dir_dupes(conn, ext=None)
    assert len(groups) == 2


def test_find_dupes_ext_multiple_extensions(conn):
    """ext filter with multiple extensions matches any of them."""
    _insert(conn, "/a/photo.jpg", "/a", "photo.jpg", 1000, "abc")
    _insert(conn, "/b/photo.jpg", "/b", "photo.jpg", 1000, "abc")
    _insert(conn, "/a/video.mp4", "/a", "video.mp4", 5000, "vvv")
    _insert(conn, "/b/video.mp4", "/b", "video.mp4", 5000, "vvv")
    _insert(conn, "/a/doc.txt", "/a", "doc.txt", 200, "ttt")
    _insert(conn, "/b/doc.txt", "/b", "doc.txt", 200, "ttt")

    groups = find_cross_dir_dupes(conn, ext={".jpg", ".mp4"})
    assert len(groups) == 2
    filenames = {g[0]["filename"] for g in groups}
    assert filenames == {"photo.jpg", "video.mp4"}


# --- find_cross_dir_dupes with min_size filter ---

def test_find_dupes_min_size_excludes_small(conn):
    """min_size: files smaller than threshold are excluded."""
    _insert(conn, "/a/small.jpg", "/a", "small.jpg", 100, "sm")
    _insert(conn, "/b/small.jpg", "/b", "small.jpg", 100, "sm")
    _insert(conn, "/a/big.jpg", "/a", "big.jpg", 5000, "bg")
    _insert(conn, "/b/big.jpg", "/b", "big.jpg", 5000, "bg")

    groups = find_cross_dir_dupes(conn, min_size=1000)
    assert len(groups) == 1
    key, files = groups[0]
    assert key["filename"] == "big.jpg"


def test_find_dupes_min_size_zero_includes_all(conn):
    """min_size=0 (default) includes all files regardless of size."""
    _insert(conn, "/a/tiny.jpg", "/a", "tiny.jpg", 1, "t1")
    _insert(conn, "/b/tiny.jpg", "/b", "tiny.jpg", 1, "t1")

    groups = find_cross_dir_dupes(conn, min_size=0)
    assert len(groups) == 1


def test_find_dupes_min_size_exact_boundary(conn):
    """Files exactly at min_size boundary are included."""
    _insert(conn, "/a/exact.jpg", "/a", "exact.jpg", 500, "ex")
    _insert(conn, "/b/exact.jpg", "/b", "exact.jpg", 500, "ex")

    groups = find_cross_dir_dupes(conn, min_size=500)
    assert len(groups) == 1


def test_find_dupes_ext_and_min_size_combined(conn):
    """ext and min_size filters work together (AND semantics)."""
    _insert(conn, "/a/photo.jpg", "/a", "photo.jpg", 100, "sm")
    _insert(conn, "/b/photo.jpg", "/b", "photo.jpg", 100, "sm")
    _insert(conn, "/a/big.jpg", "/a", "big.jpg", 5000, "bg")
    _insert(conn, "/b/big.jpg", "/b", "big.jpg", 5000, "bg")
    _insert(conn, "/a/big.txt", "/a", "big.txt", 5000, "bt")
    _insert(conn, "/b/big.txt", "/b", "big.txt", 5000, "bt")

    groups = find_cross_dir_dupes(conn, ext={".jpg"}, min_size=1000)
    assert len(groups) == 1
    key, files = groups[0]
    assert key["filename"] == "big.jpg"
...
def test_interactive_dry_run(conn, monkeypatch, tmp_path, capsys):
    ...
    total_deleted, total_errors = interactive_cleanup(conn, pairs, dry_run=True)

    # Files should still exist
    assert fa.exists()
    assert fb.exists()
    # No actual deletions counted
    assert total_deleted == 0
...
def test_interactive_dry_run_side_b(conn, monkeypatch, tmp_path, capsys):
    """dry_run=True with choice 'b' shows dry-run message for dir B."""
    fa = tmp_path / "a" / "f.jpg"
    fb = tmp_path / "b" / "f.jpg"
    fa.parent.mkdir()
    fb.parent.mkdir()
...
def test_main_no_criteria(monkeypatch):
    monkeypatch.setattr("sys.argv", ["crossdupes", "--no-name", "--no-size", "--no-md5"])
    with pytest.raises(SystemExit) as exc_info:
        from fotodedup.crossdupes import main
        main()
