"""Cleanup — find cross-directory duplicates and optionally delete them."""
...
def find_cross_dir_dupes(conn, by_name=True, by_size=True, by_md5=True):
    ...
    for row in cursor:
        ...
        params = [row[col] for col in col_map]
...
def interactive_cleanup(conn, dir_pairs):
...
def main():
    ...
    groups = find_cross_dir_dupes(conn, args.by_name, args.by_size, args.by_md5)
    dir_pairs = build_dir_pairs(groups)

    if args.delete:
        total_deleted, total_errors = interactive_cleanup(conn, dir_pairs)
